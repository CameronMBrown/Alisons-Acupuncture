<?php

/**
 * Front Page About Section
 */

$title = get_field('about_title');
$body = get_field('about_text');
$bg_image_id = get_field('about_image');
$bg_image = $bg_image_id ? wp_get_attachment_image_src($bg_image_id, 'full') : false;

?>
<section id="about" class="about-section">
  <div class="about-wr">
    <img class="about-bg" src="<?php echo esc_url($bg_image[0]); ?>" width="<?php echo esc_attr($bg_image[1]); ?>" height="<?php echo esc_attr($bg_image[2]); ?>" alt="<?php echo esc_attr($title); ?>" loading="lazy">
    <div class="cntr">
      <div class="about-content">
        <div class="about-text">
          <h2 class="text-3d-shadow slide-in-left"><?php echo esc_html($title); ?></h2>
          <p class="slide-in-left delay-short"><?php echo esc_html($body); ?></p>
        </div>
      </div>
    </div>
    <div>
</section>